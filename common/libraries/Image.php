...
class Image {
     ...
     * @return string 
     */
    public function buildString($string, $rgb = array(), $filename = '', $type = 'png', $disturb = 1, $border = true)
    {
        if (is_string($size))
            $size = explode(',', $size);
        $width = $size[0];
     ...
     * 生成图像验证码
     */
    public function buildImageVerify($randval, $width = 48, $height = 22, $type = 'png')
    {
    ...
    // 中文验证码
    public function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf', $verifyName = 'verify')
    {
