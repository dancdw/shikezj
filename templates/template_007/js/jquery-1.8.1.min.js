...
function getCookie(name) {
    ...
    if (arr != null) return unescape(arr[2], 'utf-8');
    return null;
}
...
$.lazyImg={domList:{},waitList:[],downList:[],status:0,loader:false,win:$(window),start:function(type,setting){if(this.status>1){return}this.status=1;this.type=type;this.setting=$.extend({scrollTimer:300,defaultLimit:30,attributeTag:"data-src",loaderNumber:4,offset:50,callback:null},setting);0;this.init()},init:function(){this.initDomList();if(!this.type||parseInt(this.type,10)>0){this.setRealType()}if(this.type=="scroll"||this.type=="both"){this.initDomOffsetTop();this.initEvent();this.scrollResponse()}else{this.prepend(this.domList)}},setRealType:function(){if(!this.type){this.type=this.setting.defaultLimit}if(parseInt(this.type,10)>0){this.type=(this.domListLength>=this.type)?"both":"lazy"}},initDomList:function(){this.domListLength=0;var imgs=document.images;for(var k=0,kk=imgs.length;k<kk;k++){src=imgs[k].getAttribute(this.setting.attributeTag);if(imgs[k]&&src){this.domList["init_"+k]={target:imgs[k],src:src};this.domListLength++}}},prepend:function(imgList){if(this.status==0){this.start()}imgList=this.formatImglist(imgList);this.concatList(imgList,this.waitList)},initDomOffsetTop:function(){for(var k in this.domList){if(this.domList[k]&&this.domList[k].target){this.domList[k]._offsetTop=$(this.domList[k].target).offset().top}}},initEvent:function(){this.win.on("scroll",$.lazyImg.scrollResponse);this.win.on("resize",$.lazyImg.scrollResponse)},scrollResponse:function(){var that=$.lazyImg,w=window;if(w.scrollTimer){clearTimeout(w.scrollTimer)}w.scrollTimer=w.setTimeout(function(){var scrollTop=window.pageXOffset||document.documentElement.scrollTop||document.body.scrollTop||0;var tmp=[];for(var k in that.domList){if(scrollTop+document.documentElement.clientHeight+that.setting.offset>that.domList[k]._offsetTop&&scrollTop-that.setting.offset<that.domList[k]._offsetTop+(that.domList[k].target.height||that.domList[k].target.clientHeight)){tmp.push(that.domList[k]);delete that.domList[k]}}that.prepend(tmp);tmp=null},that.setting.scrollTimer)},formatImglist:function(imgList){imgList=imgList||[];var tmp=[];for(var k in imgList){if(typeof imgList[k]=="string"){imgList[k]={src:imgList[k]}}tmp.push(imgList[k])}return tmp},concatList:function(){this.waitList=Array.prototype.concat.apply([],arguments);if(this.waitList.length>0){this.startLoad()}},startLoad:function(){if(!this.loader){this.initLoader();arguments.callee.apply(this);return}for(var k=0,kk=this.loader.length;k<kk;k++){this.loader[k].run()}},initLoader:function(){this.loader=[];for(var k=0;k<this.setting.loaderNumber;k++){this.loader.push(new this.Loader(k))}},Loader:function(key){this.key=key;this.run()},loadPrototype:{run:function(){if(this.status==1||!$.lazyImg.waitList[0]){return}this.job=$.lazyImg.waitList.shift();if(this.job&&this.job.src){this.status=1;this.img=new Image;this.img.loaderKey=this.key;this.img.onload=this.img.onerror=function(){var key=this.loaderKey;setTimeout(function(){$.lazyImg.onloadResponse(key)},0);this.onload=this.onerror=null};this.img.src=this.job.src}else{this.run()}}},onloadResponse:function(loaderKey){var loader=$.lazyImg.loader[loaderKey];if(loader.job.target&&loader.job.src){loader.job.target.setAttribute("src",loader.job.src)}loader.status=2;var tmpBool=true;if($.lazyImg.type=="scroll"||$.lazyImg.type=="both"){var tmp=[];loop:for(var k in $.lazyImg.domList){if($.lazyImg.type=="both"&&$.lazyImg.waitList.length===0){tmp.push($.lazyImg.domList[k]);delete $.lazyImg.domList[k]}tmpBool=false;break loop}Array.prototype.unshift.apply($.lazyImg.waitList,tmp);tmp=null;if(tmpBool){this.win.off("scroll",$.lazyImg.scrollResponse);this.win.off("resize",$.lazyImg.scrollResponse)}}if(tmpBool&&typeof $.lazyImg.setting.callback=="function"&&!$.lazyImg.waitList[0]&&!$.lazyImg.called&&$.lazyImg.checkAllLoaderIsReset()){$.lazyImg.setting.callback();$.lazyImg.called=true}loader.run()}};$.lazyImg.Loader.prototype=$.lazyImg.loadPrototype;var _LOAD_SCRIPT_=function(a,c){if(!a){return}if(!c){_LOAD_SCRIPT_DELAY_(a);return}var b=document.createElement("script");b.type="text/javascript";b.async=true;b.src=a;setTimeout(function(){document.getElementsByTagName("head")[0].appendChild(b)},0)};var _G_SCRIPT_lIST_=[];var _LOAD_SCRIPT_DELAY_=function(a){_G_SCRIPT_lIST_[_G_SCRIPT_lIST_.length]=a};var _RUNNING_LOAD_SCRIPT_=function(){if(_G_SCRIPT_lIST_&&_G_SCRIPT_lIST_.length>0){for(var a=0;a<_G_SCRIPT_lIST_.length;a++){_LOAD_SCRIPT_(_G_SCRIPT_lIST_[a],true)}}};if(window.addEventListener){window.addEventListener("load",_RUNNING_LOAD_SCRIPT_,false)
}else{if(window.attachEvent){window.attachEvent("onload",_RUNNING_LOAD_SCRIPT_)}else{window.onload=_RUNNING_LOAD_SCRIPT_}}function ___getPageSize(){var xScroll,yScroll;if(window.innerHeight&&window.scrollMaxY){xScroll=window.innerWidth+window.scrollMaxX;yScroll=window.innerHeight+window.scrollMaxY}else{if(document.body.scrollHeight>document.body.offsetHeight){xScroll=document.body.scrollWidth;yScroll=document.body.scrollHeight}else{xScroll=document.body.offsetWidth;yScroll=document.body.offsetHeight}}var windowWidth,windowHeight;if(self.innerHeight){if(document.documentElement.clientWidth){windowWidth=document.documentElement.clientWidth}else{windowWidth=self.innerWidth}windowHeight=self.innerHeight}else{if(document.documentElement&&document.documentElement.clientHeight){windowWidth=document.documentElement.clientWidth;windowHeight=document.documentElement.clientHeight}else{if(document.body){windowWidth=document.body.clientWidth;windowHeight=document.body.clientHeight}}}if(yScroll<windowHeight){pageHeight=windowHeight}else{pageHeight=yScroll}if(xScroll<windowWidth){pageWidth=xScroll}else{pageWidth=windowWidth}arrayPageSize=new Array(pageWidth,pageHeight,windowWidth,windowHeight);return arrayPageSize};
(function($){
    var browserV = (navigator.userAgent.toLowerCase().match( /.+(?:ie)[\/: ]([\d.]+)/ ) || [0,'0'])[1];
    var browserVersion = (browserV == "7.0" || browserV == "6.0")? false : true;
