...
<div class="box">
    ...
    <div class="b_sub fl">
        ...
        <div class="sort">
            <a href="<!--{link m='games'}-->" <!--{if $cate_id>0}--><!--{else}-->class="sort_on"<!--{/if}-->>全部</a>
            <!--{appnav parent=2}-->
            <a href="<!--{link m='games' cate_id=$appnav.cate_id}-->"  <!--{if $cate_id == $appnav.cate_id}-->class="sort_on"<!--{/if}-->><!--{$appnav.cname}--></a>
            <!--{/appnav}-->
        </div>
    ...
    <div class="b_main fr">
        <div class="sub_tit">
            <div class="order">
                <a <!--{if $order == 0}--> class="order_on" <!--{/if}--> href="<!--{link m='games' cate_id=$cate_id page=$page}-->">更新时间<s></s></a>
                <a <!--{if $order == 1}--> class="order_on" <!--{/if}--> href="<!--{link m='games' cate_id=$cate_id order=1 page=$page}-->">下载次数<s></s></a>
            </div>
            全部软件
        </div>
        <ul class="app_list">
            <!--{applist page=$page order=$order_by parent=2 id=$cate_id per_page=18}-->
            <li>
        ...
        <!--{pagebar name='applist' page=$page parent=2 id=$cate_id order=$order per_page=18}-->
    </div>
    <!-- 右侧软件列表 -->
</div>
