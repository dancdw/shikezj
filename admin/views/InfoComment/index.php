...
<script>
    function comment_del(comment_id){
        show_msg("确认删除？", true, function(){
            $.post(build_url('InfoComment', 'delete'), {"comment_id": comment_id}, function(result) {
                if(result.code != 0){
                    show_msg(result.msg);
                }else{
                    location.reload();
    ...
    function comment_delete_all() {
        ...
        show_msg("您确定要删除这些评论吗？", true, function(){
            ...
            $('.cklist:checked').each(function(index){
                var o = $(this);
                var count = index + 1;
                $.post(build_url('InfoComment', 'delete'), {"comment_id": o.val()}, function(result) {
    ...
    function comment_update(comment_id, status) {
        $.post(build_url('InfoComment', 'update'), {"comment_id": comment_id, "status": status}, function(result) {
            if(result.code != 0){
                show_msg(result.msg);
            }else{
                location.reload();
    ...
    function comment_update_all(status){
        ...
        show_msg("您确定要更改这些评论的当前状态吗？", true, function(){
            ...
            $('.cklist:checked').each(function(index){
                var o = $(this);
                var count = index + 1;
                $.post(build_url('InfoComment', 'update'), {"comment_id": o.val(),"status": status}, function(result) {
