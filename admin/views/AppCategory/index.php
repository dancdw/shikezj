    ...
    <div class="mbody">
        <div class="main mhead clearfix">
            <div class="mt10" style="height:28px; margin-bottom:10px;">
                <div class="l">
                    <a href="<?php echo build_url('AppCategory', 'content'); ?>" class="but2">添加分类</a>
                </div>
            </div>
        </div>
            ...
            <form action="?m=save_corder" name="form_corder" method="post">
                <table class="tb tbs" id='list' border="1" bordercolor="#cee1ee">
                    <tr class="tr-title">
                        ...
                        <th align='center' width="15%">分类名称</th>
                        <th align='center' width="10%">应用个数</th>
                        <th align='center' width="10%">分类logo</th>
                    ...
                    <?php if (is_array($list) && sizeof($list) > 0) : ?>
                        <?php foreach ($list as $value) : ?>
                            <tr>
                                ...
                                <td><a href="<?php echo build_url('AppCategory', 'content', array('cate_id' => $value['cate_id'])); ?>"><?php echo $value['cname']; ?></a></td>
                                <td><?php echo $value['cate_count']; ?></td>
                                <td><?php echo !empty($value['cate_logo']) ? '<img src="'.$value['cate_logo'].'" width="36" height="36" />' : ''; ?></td>
                                <td><?php echo $value['cname_py']; ?></td>
                                <td><?php echo $value['cat_show'] == 1 ? '显示' : '不显示'; ?></td>
                                <td>
                                    <a class="but2 but2s" href="<?php echo build_url('AppCategory', 'content', array('cate_id' => $value['cate_id'])); ?>" class="btn2">修改</a>
...
<script>
    function category_del(cate_id){
        show_msg("只有分类下面没有应用才能被删除，确认吗？", true, function(){
            $.post(build_url('AppCategory', 'delete'), {"cate_id": cate_id}, function(result) {
                if(result.code != 0){
                    alert(result.msg);
                }else{
                    location.reload();
