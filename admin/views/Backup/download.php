            ...
            <table class="tb mt10" border="1" bordercolor="#cee1ee">
                ...
                <?php if (is_array($list) && sizeof($list) > 0) : ?>
                    <?php foreach ($list as $value) : ?>
                        <tr>
                            <td>
                                <a href="<?php echo build_url('Backup', 'file', array('name' => $name, 'file' => substr($value['name'], 0, -4))); ?>">
                                    <?php echo $value['name']; ?>
                                </a>
                            </td>
                            <td><?php echo substr($value['name'], 0, -4); ?></td>
                            <td><?php echo $value['size']; ?> bytes</td>
                            <td><?php echo date('Y-m-d H:i:s',$value['date']); ?></td>
                            <td>
                                <a class="but2 but2s" href="<?php echo build_url('Backup', 'file', array('name' => $name, 'file' => substr($value['name'], 0, -4))); ?>">下载</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
