...
class MY_Controller extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        if (in_array($this->input->get($c), array("", "index", "Index"))) {
            return;
        }
    ...
    protected function is_user_ajax()
    {
        $site = $this->_site;
        if (empty($site['auth_user']) || empty($site['auth_code']) || empty($site['version']) || empty($site['login_time'])) {
            $this->output->error_json(999, "只有VIP会员才可以使用此功能。请到 https://www.711cms.com 开通会员。");
        } else {
            $args = $this->__A($site['auth_user'], $site['auth_code'], $site['version'], $site['login_time']);
            if (!$this->__X($site['user_data'], $args)) {
                $this->output->error_json(999, "只有VIP会员才可以使用此功能。请到 https://www.711cms.com 开通会员。");
            }
        }
    }
    
    protected function is_user_ajax2()
    {
        $site = $this->_site;
        if (empty($site['auth_user']) || empty($site['auth_code']) || empty($site['version']) || empty($site['login_time'])) {
           return false;
    ...
    protected function getTemplatesDir($type = '')
    {
        ...
        foreach ($templates as $key => $value) {
            ...
            if ($type) {
                ...
                if (substr($key, 0, 7) == 'mobile_' || substr($key, 0, 7) == 'wechat_' || substr($key, 0, 9) == 'template_') {
                    $t[] = $value;
                }
            }
        }
