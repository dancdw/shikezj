...
class App extends MY_Controller {
    ...
    public function save() {
        ...
        $app_stitle = $this->input->post('app_stitle');
        $app_update_time = $this->input->post('app_update_time', true);
        $app_type = intval($this->input->post('app_type'));
        $app_hot = intval($this->input->post('app_hot'));
        $app_new = intval($this->input->post('app_new'));
        ...
        $app_recomment_url = $this->input->post('app_recomment_url', true);
        $app_video_url = $this->input->post('app_video_url', true);
        ...
        $app_seodesc = $this->input->post('app_seodesc');
        ...
        if ($app_id > 0) {
            ...
            if ($app_hot) {
                $data['app_hot'] = $app_hot;
            }
            if ($app_new) {
                $data['app_new'] = $app_new;
            }
            if ($app_logo) {
            ...
            if ($app_recomment) {
                $data['app_recomment_url'] = $app_recomment_url;
            }
            if ($app_video_url) {
                $data['app_video_url'] = $app_video_url;
            }
            $data['uid'] = $this->_uid;
            if ($app_comments) {
            ...
            if ($app_seodesc) {
                $data['app_seodesc'] = htmlspecialchars($app_seodesc);
            }
            if ($app_seokey) {
        ...
        $insert_id = $this->m_App->createApp(array(
            ...
            "app_recomment_url" => $app_recomment_url,
