...
function smarty_function_pagebar($params, &$smarty)
{
    ...
    $_site = $ci->_site;
    $rewrite = isset($_site['is_site_rewrite']) && $_site['is_site_rewrite'] == 1 ? true : false;
    ...
    if ($name == 'infolist') {
        ...
        $names = array('infolist' => 'list_info');
        ...
        $config['base_url'] = build_url('index', "content_special", $arg);
        ...
        $area_arr = explode(',',$area_ids);
        $count = count($area_arr);
        $config['base_url'] = build_url('index', "necessary_list", $arg);
        ...
        $area_arr = explode(',',$area_ids);
        $count = count($area_arr);
        $config['base_url'] = build_url('index', "list_recommend", $arg);
